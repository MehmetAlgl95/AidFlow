...
public class NGOAidManagementSystem {
    ...
    private static void showRegistrationScreen() {
        ...
        registerFrame.getContentPane().add(BorderLayout.SOUTH, registerButton);
    ...
    private static void donateAid(String username) {
        ...
        donateButton.addActionListener(e -> {
            ...
            if (!amount.isEmpty() && !type.isEmpty()) {
                saveDonation(username, amount, type);
                JOptionPane.showMessageDialog(donateFrame, "Donation of " + amount + " " + type + " made.");
                donateFrame.dispose();
            } else {
                JOptionPane.showMessageDialog(donateFrame, "Amount and Type cannot be empty");
            }
        });

        donateFrame.getContentPane().add(BorderLayout.CENTER, panel);
        donateFrame.getContentPane().add(BorderLayout.SOUTH, donateButton);
        donateFrame.setVisible(true);
    }

    private static void saveDonation(String username, String amount, String type) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(username + "_donations.txt", true))) {
            bw.write(amount + "," + type);
            bw.newLine();
        } catch (IOException e) {
            e.printStackTrace();
        }
